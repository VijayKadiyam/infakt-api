...
class UserAppointmentLetterTest extends TestCase
{
  ...
  {
    $this->json('post', '/api/user_appointment_letters')
      ->assertStatus(401); 
  }
  ...
  {
    $this->json('post', '/api/user_appointment_letters', [], $this->headers)
      ->assertStatus(422)
      ->assertExactJson([
          "errors"  =>  [
  ...
  {
    $this->disableEH();
    $this->json('post', '/api/user_appointment_letters', $this->payload, $this->headers)
      ->assertStatus(201)
      ->assertJson([
          'data'   =>[
            'letter'        =>  'Letter 2'
  ...
  {
    $this->json('get', "/api/user_appointment_letters/1", [], $this->headers)
      ->assertStatus(200)
      ->assertJson([
          'data'  => [
            'letter'        =>  'Letter 1',         
  ...
  function update_single_letter()
  {
    ...
    $this->json('patch', '/api/user_appointment_letters/1', $payload, $this->headers)
      ->assertStatus(200)
      ->assertJson([
          'data'    => [
            'letter'        =>  'Letter 1 Updated',
